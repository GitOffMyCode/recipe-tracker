...
server.post("/recipes/", function (req, res, next) {
  const error = validateRecipe(req.body); // only validating on server.post
  if (error) {
    res.status(400).send(error);
  } else {
    req.body.slug = createSlug(req.body.recipeTitle); // !!! BUG WAS HERE "title" instead of recipeTitle
    next();
  }
});
...
function validateRecipe(recipe) {
  if (!recipe.recipeTitle) return "Title is required.";
  if (!recipe.chefId) return "Chef is required.";
  if (!recipe.categoryId) return "Category is required.";
  return "";
}
